...
#ifndef  _routerVcMP_cc_INC
#define  _routerVcMP_cc_INC
...
RouterVcMP::RouterVcMP ()
{
    name = "RouterVcMP" ;
    ticking = false;
}  /* -----  end of method RouterVcMP::RouterVcMP  (constructor)  ----- */
...
void
RouterVcMP::init (uint p, uint v, uint cr, uint bs)
{
    ports =p;
    vcs =v;
    credits =cr;
...
void
RouterVcMP::process_event ( IrisEvent* e )
{
    switch(e->type)
    {
        ...
        case DETECT_DEADLOCK_EVENT:
            ...
            break;
        default:
            _DBG("RouterVcMP:: Unk event exception %d", e->type);
            break;
    }
    return ;
}		/* -----  end of function RouterVcMP::process_event  ----- */

void 
RouterVcMP::handle_detect_deadlock_event(IrisEvent* e )
{
    for ( uint i=0; i<ports; i++)
        for ( uint j=0; j<vcs; j++)
        {
...
void
RouterVcMP::handle_link_arrival_event ( IrisEvent* e )
{
    ...
    delete e;
    return ;
}		/* -----  end of function RouterVcMP::handle_link_arrival_event  ----- */

void
RouterVcMP::do_input_buffering()
{
    for( uint i=0; i<ports*vcs; i++)
    {
...
void
RouterVcMP::do_switch_traversal()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RouterVcMP::do_switch_allocation()
{
...
void
RouterVcMP::do_virtual_channel_allocation()
{
...
void
RouterVcMP::request_switch_allocation()
{
    for( uint i=0; i<(ports*vcs); i++)
...
void
RouterVcMP::handle_tick_event ( IrisEvent* e )
{
    ticking = false;
    do_switch_traversal();
...
}		/* -----  end of function RouterVcMP::handle_input_arbitration_event  ----- */

string
RouterVcMP::toString() const
{
    stringstream str;
    str << "RouterVcMP"
        << "\t addr: " << address
...
void
RouterVcMP::send_credit_back(uint i)
{
